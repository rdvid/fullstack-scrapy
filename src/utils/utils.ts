    ...
    try {     
        ...
        $('div.puis-card-container')
            .each((i, element)=>{
                ...
                const reviewsCount = $(element).find('span.rush-component span.a-size-base.s-underline-text').first().text().replace(',', '.');
                const imageUrl = $(element).find('div.s-image-square-aspect>img').prop('src');
                let price = amount ? Number.parseFloat(amount) : 0;
